...
- **Secrets never touch the client.** Google keys, `CALENDAR_ID`, `TURNSTILE_SECRET` live only as Cloudflare Pages secrets (local: `.dev.vars`, git-ignored). Only the Turnstile *site key* is public in the frontend. Never use `VITE_`/`PUBLIC_`-style bundled env for secrets.
